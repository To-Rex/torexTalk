...
@router.put("/edit_response/{session_name}")
async def edit_response(session_name: str, request: ResponseRequest, question: str = None, response_index: int = None):
    if not question or response_index is None:
        raise HTTPException(status_code=400, detail="Question and response_index parameters are required")
    if not await modify_session_data(session_name, "edit_response", question=question, response_index=response_index, response=request.response):
        raise HTTPException(status_code=404, detail="Response not found")
    return {"message": f"Response {response_index} edited for question '{question}' in {session_name}"}
...
@router.delete("/delete_response/{session_name}")
async def delete_response(session_name: str, question: str = None, response_index: int = None):
    if not question or response_index is None:
        raise HTTPException(status_code=400, detail="Question and response_index parameters are required")
    success, has_responses = await modify_session_data(session_name, "delete_response", question=question, response_index=response_index)
    if not success:
        raise HTTPException(status_code=404, detail="Response not found")
    if not has_responses:
        raise HTTPException(status_code=400, detail="Cannot delete last response")
    return {"message": f"Response {response_index} deleted from question '{question}' in {session_name}"}
